...
struct
	...
	let server_channel port =
		let lv = ((ref []) :'a list ref) in
		let respond out_ch answ =
		  Marshal.to_channel out_ch answ [ Marshal.Closures ];flush out_ch
		in
	...
	let rec get c ()=
		...
		let send_request_and_receive s = 
		  let out_ch = out_channel_of_descr s and in_ch = in_channel_of_descr s in 
		    ...
		    flush out_ch;
		    let answ = ((Marshal.from_channel in_ch) : 'a answer) in
		    match answ with
		      |SUCCESS v -> v
	...
	let run e =
		trace "run";
		(*let list_server = ref([]) in
		for p = !port_min to !port_max
		do
		list_server := (fun () -> server_channel p)::!list_server;
		done;
		create_servers !list_server;
		port_min := !port_max +1;
		trace "will now run";*)
		e ()
